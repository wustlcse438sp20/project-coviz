...
package com.example.project_coviz.s2;
...
public final strictfp class S2Loop implements S2Region, Comparable<S2Loop> {
  ...
  public boolean contains(S2Point p) {
    ...
    if (numVertices < 2000) {
      ...
      S2EdgeIndex.DataEdgeIterator it = getEdgeIterator(numVertices);
      int previousIndex = -2;
  ...
  public boolean isValid() {
    ...
    for (int a1 = 0; a1 < numVertices; a1++) {
      ...
      S2EdgeUtil.EdgeCrosser crosser = new S2EdgeUtil.EdgeCrosser(vertex(a1), vertex(a2), vertex(0));
      int previousIndex = -2;
  ...
  private int checkEdgeCrossings(S2Loop b, S2EdgeUtil.WedgeRelation relation) {
    S2EdgeIndex.DataEdgeIterator it = getEdgeIterator(b.numVertices);
    int result = 1;
